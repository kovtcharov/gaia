...
class PreferenceToolsMixin:
    ...
    def _register_preference_tools(self) -> None:
        ...
        def set_priority_sender(email: str, confirmed: bool = False) -> str:
            ...
            output — ``pre_scan_inbox`` uses it to put this sender's mail
            first within each attention section. The category (urgent,
            needs_response, ...) is still decided entirely by the message
            content; this tool never overrides it.
