...
<Tabs>
  ...
  <Tab title="Package Config">
    ...
    name = "doc-qa-agent"
    version = "1.0.0"
    # ChatAgent is installed from source while PyPI publishing is paused.
    dependencies = [
        "amd-gaia[rag]",
        "gaia-agent-chat @ git+https://github.com/amd/gaia.git#subdirectory=hub/agents/chat/python",
    ]
    ...
    Install with: `uv pip install -e .` (requires Git). The direct source dependency
    supplies `ChatAgent`; the core `rag` extra supplies document-processing dependencies.
  </Tab>
...
<AccordionGroup>
  <Accordion title="No documents indexed" icon="circle-xmark">
    ...
    **Common causes:**
    - Missing RAG dependencies (`uv pip install "amd-gaia[rag]"`)
    - Incorrect file path
    - File not readable
  </Accordion>
