...
To enable combined prompt (for cost/speed optimization), set
`"combined_prompt": true` in parameters:
...
`gaia eval tasks` gives the flagship GaiaAgent real work: fix a bug and add a regression test, add a feature, turn a CSV into a weekly report, explain a file. It scores what the finished project **does**, not what the agent says it did. CI runs it on pull requests that touch the agent, and can fail the build when passes, quality, honesty, tokens, steps or runtime fall outside the committed expectations.
...
- A **coding task** passes when the project's own tests pass *and* a probe run inside the finished project confirms the behavior. For example, the bug fix must accept lowercase `z`, and the weekly report must match an independent recomputation row for row.
- A **question task** passes when the answer names every fact it must.
- A **judge** (Claude, with no tools, and not told whether the task passed) reads the original project, the agent's answer and a diff of its changes. It grades four things from 1 to 5: following the instructions, quality of the work, reasoning, and whether the answer claims anything the agent never did.

### Suites

| Suite | Tasks | Where it runs |
|---|---|---|
| `core` | Explain a file · bug fix with a regression test · weekly CSV report · config from an environment variable | Every CI run |
| `full` | `core` plus: refactor to one shared implementation · code review · review a patch · write tests for an untested module (mutation-checked) | `workflow_dispatch` with `task_suite: full` |

Tasks and suites live in `eval/tasks/tasks.json`. Every coding task has a reference solution in `tests/unit/eval/test_flagship_tasks.py`. The tests prove each task fails on the untouched project and passes a correct solution.

### Commands

| Command | What it does |
|---|---|
| `gaia eval tasks run [--suite core] [--model M] [--out DIR] [--no-judge]` | Runs the flagship on each task and writes `scorecard.json`, plus each task's `transcript.json` and `workspace.diff`. Judges afterwards unless `--no-judge`. |
...
| `gaia eval tasks gate DIR [--expect FILE] [--enforce] [--propose FILE]` | Compares the run with its expectations. Reports by default; `--enforce` exits 1 on a miss. |
...
```json
{
  ...
  "measured": {"passed": 4, "quality": 3.6, "misreported": 1,
               "total_tokens": 250000, "steps": 30, "wall_seconds": 600},
  "tasks": [{"id": "02-bugfix", "result": "PASS", "steps": 8,
...
- `--propose FILE` writes this file from a run, with headroom for one noisy run: one task may flip, quality may drop 0.5, tokens and steps may grow 35%, and runtime may grow 50%. Runtime includes the model load, so it swings more than tokens do.
