...
<Warning>
  ...
  **Use the new framework instead:**
  ...
  - [Eval Scorecard](/reference/eval-scorecard) — the per-agent release scorecard; the **flagship gaia agent** is scored by `gaia eval agent --agent-type gaia` over the 12-category `eval/scenarios/gaia_*` corpus (aggregate = judged pass rate), published at `hub/agents/gaia/npm/SCORECARD.md` and gated at release
</Warning>
