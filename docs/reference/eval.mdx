...
<Warning>
  The dataset and results contain your own prompts and the answers Claude Code gave you. `eval/results/sessions*/` is gitignored for that reason; don't commit it or upload it as a CI artifact. A custom `--out` outside `eval/results/` is **not** covered by that ignore rule.
</Warning>
