...
There are two `publish_to_r2.py` scripts — the email agent's and the gaia
flagship's — and every case runs against both, so a fix that lands in one copy
and not the other fails here rather than during a release. Narrow it with
`--publisher gaia`.
