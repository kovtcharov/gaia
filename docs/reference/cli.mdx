...
## Durable service commands

`gaia-agent --controller` starts the single-writer durable service.
`gaia-agent --durable-client` supports capabilities, create-session, run, status,
tail, cancel, interaction and answer. See [durable service](/guides/durable-service)
for credentials, stable retry identities, complete approval review and recovery.
