...
| `full_access` | `false` | Start every session with confirmation prompts off — see [Full access](#full-access) |

### Full access

By default the agent asks before it runs a shell command or writes a file. Turn
that off for every session with:
...
| `gaia --full-access=false` | skip it for one launch |
