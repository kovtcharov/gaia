...
under `tests/fixtures/gaia/`. Tier tags (`t1_basic` → `t4_adversarial`) grade
...
| `gaia_shell` | Shell execution and the gh permission gate: ALLOW runs promptless, CONFIRM surfaces the exact command, REFUSE dies in prose, denials reported as denials |
