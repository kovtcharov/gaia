...
Our focus for the next few quarters, driven by community feedback and votes.
...
graph LR
    Q1A["Q1 2026<br/>Lightweight Installer<br/><small>One-command install</small>"]
    Q1B["Q1 2026<br/>Chat Desktop UI<br/><small>Privacy-first RAG</small>"]
    Q1C["Q1 2026<br/>MCP Ecosystem<br/><small>Client + Docs Server</small>"]
    Q2["Q2 2026<br/>TBD<br/><small>Community-driven</small>"]

    Q1A -.-> Q1B
    Q1B -.-> Q1C
    Q1C --> Q2

    style Q1A fill:#ED1C24,stroke:#C8171E,stroke-width:2px,color:#fff
    ...
    style Q2 fill:#2d2d2d,stroke:#1a1a1a,stroke-width:2px,color:#fff

    linkStyle 0 stroke:#ED1C24,stroke-width:2px,stroke-dasharray: 5 5
    linkStyle 1 stroke:#ED1C24,stroke-width:2px,stroke-dasharray: 5 5
...
### Q1 2026: MCP Ecosystem

Two complementary features that unlock the full power of the Model Context Protocol:

#### MCP Client Support

**Connect GAIA agents to any MCP server**

Enable any agent to use tools from external MCP servers - filesystem, databases, GitHub, Slack, and hundreds more:
...
class MyAgent(MCPClientMixin, Agent):
    def __init__(self):
        super().__init__()
        self.connect_mcp_server("filesystem", "npx @mcp/server-filesystem /home")
        self.connect_mcp_server("github", "npx @mcp/server-github")
        # MCP tools auto-register as agent tools!
```

Key capabilities:
- Universal MCP server connectivity (stdio, HTTP, WebSocket)
...
An MCP server that gives AI coding assistants intelligent access to GAIA SDK documentation:
