...
| **CodeAgent** | Code | Code generation and editing | `agents/code/` (removed) |
| **BlenderAgent** | Creative | 3D scene automation | `agents/blender/` (removed) |
| **JiraAgent** | Productivity | Issue management with NLP | `agents/jira/` (removed) |
| **DockerAgent** | DevOps | Container management | `agents/docker/` (removed) |
| **MedicalIntakeAgent** | Healthcare | Form processing with VLM | `hub/agents/emr/python/` (removed) |
| **RoutingAgent** | Infrastructure | Intelligent agent selection | `agents/routing/` (removed) |
| **SDAgent** | Creative | Image generation with SD + VLM | `agents/sd/` (removed) |
