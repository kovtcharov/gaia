...
class MCPAgent(Agent):
    ...
    Example:
        >>> class FileToolsAgent(MCPAgent, Agent):
...
class HybridAgent(MCPAgent, ApiAgent, Agent):
    # MCP methods
    def get_mcp_tool_definitions(self):
        return [...]
...
class CodeAnalyzerAgent(MCPAgent, ApiAgent, Agent):
...
# 2. API: POST /v1/chat/completions with model="code-analyzer"
#    (after registering it in AGENT_MODELS — see spec/api-server)
```
