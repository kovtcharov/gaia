...
- Multi-app support via `app.config.json` — currently shipping the flagship Agent UI (`webui`)
...
flowchart TD
    ...
    style E fill:#A87B2D,stroke:#A87B2D,stroke-width:2px,color:#fff
...
flowchart TD
    ...
    style A fill:#E2A33E,stroke:#A87B2D,stroke-width:2px,color:#1a1a1a,rx:10,ry:10
    style B fill:#EFC480,stroke:#E2A33E,stroke-width:2px,color:#1a1a1a,rx:10,ry:10
    style C fill:#EFC480,stroke:#E2A33E,stroke-width:2px,color:#1a1a1a,rx:10,ry:10
    ...
    linkStyle 0,1,2,3,4 stroke:#E2A33E,stroke-width:2px
```
...
| **apps/** | `webui/` (flagship Agent UI), `example/webui/` - each with `app.config.json` |
...
**Example:** `src/gaia/apps/webui/app.config.json` (the flagship Agent UI)
```json
{
  "name": "agent-ui",
...
```bash
# Required
GAIA_APP_NAME=webui|example  # App to launch
...
describe('App Configuration', () => {
  it('should load valid app.config.json', () => {
    ...
    assert(config.backend.port, 'backend.port is required');
  });
...
describe('App Launch', function() {
  ...
  before(async () => {
    app = new Application({
      ...
      env: { GAIA_APP_NAME: 'webui', GAIA_APP_MODE: 'development' }
    });
  ...
  it('should load app URL', async () => {
    ...
    assert(url.includes('localhost:4200'));
  });

  it('should have correct title', async () => {
    const title = await app.client.getTitle();
    assert(title.includes('GAIA Agent UI'));
  });
});
```
...
cat > src/gaia/apps/myapp/webui/app.config.json <<EOF
{
  ...
  "backend": {
    "apiUrl": "http://localhost:13305/api/v1",
    "mcpPort": 8765
  }
}
EOF
...
```bash
# Run with dev tools
GAIA_APP_NAME=webui GAIA_APP_MODE=development npm start
```
...
- [chat-agent](/spec/chat-agent) - Backs the flagship Agent UI's Electron shell
