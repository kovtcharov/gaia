...
class ApiAgent(Agent):
    ...
    Example:
        >>> class WeatherAgent(ApiAgent, Agent):
        ...     def get_model_id(self) -> str:
        ...         return "gaia-weather-agent"
        ...
    ...
    def get_model_id(self) -> str:
        ...
        Returns:
            Model ID string (e.g., "gaia-weather", "gaia-search")

        Example:
            WeatherAgent -> gaia-weather
            SearchAgent -> gaia-search
            TranslateAgent -> gaia-translate
...
class SearchAgent(MCPAgent, ApiAgent, Agent):
    def get_model_id(self) -> str:
        return "gaia-search"
...
def test_naming_conventions():
    ...
    class SearchAgent(ApiAgent, Agent):
        def _get_system_prompt(self):
            return "Search"
    ...
    weather_agent = WeatherAgent(silent_mode=True)
    search_agent = SearchAgent(silent_mode=True)

    assert weather_agent.get_model_id() == "gaia-weather"
    assert search_agent.get_model_id() == "gaia-search"
```
...
class WeatherAgent(ApiAgent, Agent):
    ...
    def _get_system_prompt(self) -> str:
        return "You are a weather lookup assistant."
    ...
    def get_model_info(self) -> Dict[str, Any]:
        return {
            "max_input_tokens": 32768,
            "max_output_tokens": 8192,
            "description": "Weather lookup agent with forecast and alert tools",
            "supports_tools": True,
...
# {"model": "gaia-weather-agent", "messages": [...]}
```
...
class SearchAgent(MCPAgent, ApiAgent, Agent):
    """Search agent exposed via both MCP and OpenAI API."""
